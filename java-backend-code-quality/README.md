...
用于编写、修改或检查 Java 后端代码。默认只检查当前任务改动的代码，并根据代码内容读取命名、方法设计、注释、异常和生命周期等详细规则。
...
检查器不会判断自然语言是否清楚、处理阶段是否说明完整、私有方法或复用型异常文案常量是否应该写回使用位置、错误原因是否应拆分、恢复动作是否有效、错误码是否匹配或是否泄露敏感信息。这些问题由 Skill 结合代码含义检查。
