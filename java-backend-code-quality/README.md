...
- 非显而易见的处理阶段、调用原因、状态变化和异常后果需要局部意图注释
...
退出码为 `0` 表示通过，`1` 表示发现违规，`2` 表示参数、输入或环境错误。重试、吞异常、补偿和降级语义仍需要结合源码进行人工语义检查
...
当前测试集包含 61 个用例。在未授予 Windows 符号链接权限的环境中，两个越界符号链接用例会跳过
