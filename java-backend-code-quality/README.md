...
[`evals/semantic-cases.json`](evals/semantic-cases.json) 覆盖短私有方法、阶段注释、Map 重复策略、第三方错误、异常降级、JavaDoc、结构选择、关联字段、旧日志意图、证据冲突和工具改写后的最终回读。每个场景记录必须出现和不能出现的决策，不比较固定文案。
