...
- add top-level intent comments when names and types do not reveal a stage's reason, precondition, consumer, order, compatibility rule, or failure consequence. Describe the accepted current behavior; mention a superseded path, legacy field, or unused input only when its current compatibility role or exclusion is needed to prevent a wrong change. Treat adjacent calls as separate stages when responsibility, handoff, side effects, ordering, or failure boundaries differ
- compare compact private helpers, fields, and constants with writing their logic or value at the use sites. Prefer the clearer call site; keep short helpers that combine low-level checks into a useful business or protocol concept with an accurate contract. Expand mere lookup or forwarding wrappers that hide inputs, null handling, the real delegate, or error text. Neither length nor reuse count decides
