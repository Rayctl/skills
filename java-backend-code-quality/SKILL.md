...
- keep an extracted method when it owns a stable stage or policy; split methods that mix orchestration, transformation, side effects, and cleanup into an opaque block
- for a changed private method with one source call site, compare the extracted and inlined forms; retain it only when its basic contract is clear at the call site without an implementation jump and it exposes a stable business policy, isolates non-trivial branching or an algorithm, owns non-trivial transaction demarcation, resource lifecycle, side-effect ordering, compensation, or failure policy, or satisfies a framework contract
- inline one-call-site helpers that only forward arguments, rename a few linear statements, add a routine null check, construct a fixed exception, or exist to reduce line count; wrapping one persistence or remote call does not create a boundary merely because the callee performs I/O or mutation; prefer local intermediate names and stage comments when they keep the behavior readable in place, and verify framework, annotation, reflection, or serialization use before treating textual references as call sites
